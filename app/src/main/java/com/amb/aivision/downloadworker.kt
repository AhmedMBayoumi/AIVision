...
class DownloadWorker(context: Context, params: WorkerParameters) :
    ...
    override suspend fun doWork(): Result {
        ...
        val extraDataFileNames = inputData.getString(KEY_MODEL_EXTRA_DATA_DOWNLOAD_FILE_NAMES)?.split(",") ?: listOf()
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                for (index in extraDataFileUrls.indices) {
                    allFiles.add(UrlAndFileName(url = extraDataFileUrls[index], fileName = extraDataFileNames[index]))
                }
                ...
                for (file in allFiles) {
                    ...
                    val outputDir = File(externalFilesDir, modelDir)
                    if (!outputDir.exists()) {
                        outputDir.mkdirs()
                    }
                    ...
                    if (outputFileBytes > 0) {
                        Log.d(TAG, "File '${file.fileName}' partial size: ${outputFileBytes}. Trying to resume download")
                    ...
                    while (inputStream.read(buffer).also { bytesRead = it } != -1) {
                        ...
                        if (curTs - lastSetProgressTs > 200) {
                            ...
                            setForeground(createForegroundInfo(progress = (downloadedBytes * 100 / totalBytes).toInt(), modelName = modelName))
                    ...
                    if (isZip && unzippedDir != null) {
                        ...
                        val destDir = File(externalFilesDir, listOf(modelDir, unzippedDir).joinToString(File.separator))
                        if (!destDir.exists()) {
                            destDir.mkdirs()
                        }

                        val zipFilePath = File(outputDir, file.fileName).absolutePath
    ...
    companion object {
        ...
        const val KEY_MODEL_DOWNLOAD_FILE_NAME = "download_file_name"
        const val KEY_MODEL_DOWNLOAD_MODEL_DIR = "model_dir"
        const val KEY_MODEL_IS_ZIP = "is_zip"
        const val KEY_MODEL_UNZIPPED_DIR = "unzipped_dir"
        const val KEY_MODEL_EXTRA_DATA_URLS = "extra_data_urls"
        const val KEY_MODEL_EXTRA_DATA_DOWNLOAD_FILE_NAMES = "extra_data_download_file_names"
        const val KEY_MODEL_TOTAL_BYTES = "total_bytes"
        const val KEY_MODEL_DOWNLOAD_ACCESS_TOKEN = "access_token"
        const val KEY_MODEL_DOWNLOAD_RECEIVED_BYTES = "received_bytes"
        const val KEY_MODEL_DOWNLOAD_RATE = "download_rate"
        const val KEY_MODEL_DOWNLOAD_REMAINING_MS = "remaining_ms"
        const val KEY_MODEL_START_UNZIPPING = "start_unzipping"
        const val KEY_MODEL_DOWNLOAD_ERROR_MESSAGE = "error_message"
    }
}